...
const newBlock = `) : selectedProject.id === 'wilo' ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    >
                      <img src={selectedProject.images[0]} style={{ width: '100%', height: 'auto', maxHeight: '550px', objectFit: 'cover', objectPosition: 'center', borderRadius: '12px' }} alt="Wilo Collage Hero" />
                    ...
                    {/* Row 1: The Professional Zones (50/50 split) */}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                      {[1, 2].map((idx) => (
                        ...
                        >
                          <img src={selectedProject.images[idx]} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s ease' }} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'} alt="Professional Zone" />
                        </motion.div>
                      ))}
                    </div>

                    {/* Row 2: Culture & Overview (50/50 split) */}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                      {[3, 4].map((idx) => (
                        <motion.div 
                          key={idx} 
                          ...
                          onClick={() => setLightboxImage(selectedProject.images[idx])}
                        >
                          <img src={selectedProject.images[idx]} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s ease' }} onMouseOver={(e) => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={(e) => e.currentTarget.style.transform = 'scale(1)'} alt="Culture Overview" />
                        </motion.div>
                      ))}
                    </div>
                  </div>
...
if (regex.test(content)) {
  ...
  console.log("Successfully added Wilo layout to projects-portfolio.tsx");
} else {
